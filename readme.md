## Минуточку внимания!
Поскольку администрация ВКонтакте решила 15 февраля [закрыть доступ к API сообщений](https://vk.com/dev/messages_api), данный скрипт рискует потерять часть своего функционала. Пройдёт модерацию разработчик библиотеки [vk_api](https://github.com/python273/vk_api) или я, или всё же придётся придумывать обходной путь для получения сообщений - пока неизвестно. Тем не менее, всегда стоит надеяться на лучшее :з
